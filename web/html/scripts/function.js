...
$(function(){
    ...
    function checkItem(dom){
        ...
        switch(name){
            ...
            case "PAIKECOURSENAME":
                if(value == "") {
                    errorMessage = "课程名不能为空";
                }else if(value.length>50){
                    errorMessage = "课程名长度最长50个汉字";
                }else{
                    isError = false ;
                }
                break;
            case "PAIKETEACHER":
                if(value == "") {
                    errorMessage = "任课老师不能为空";
                }else if(value.length>20){
                    errorMessage = "任课老师长度最长20个汉字";
                }else{
                    isError = false ;
                }
                break;
            case "PAIKEWEEKS":
                if(value == "") {
                    errorMessage = "上课周数不能为空";
                }else if(value.length>20){
                    errorMessage = "上课周数长度最长20个汉字";
                }else{
                    isError = false ;
                }
                break;
            case "PAIKEJIESHU":
                if((value == "")||!(/^[0-9]*[1-9][0-9]*$/.test(value))) {
                    errorMessage = "不能为空且只能为正整数，最好1~2位正整数";
                }else{
                    isError = false ;
                }
                break;
            case "PAIKEWEEKDAY":
                if((value == "")||!(/^[0-9]*[1-9][0-9]*$/.test(value))) {
                    errorMessage = "不能为空且只能为正整数，最好是数字1~7";
                }else{
                    isError = false ;
                }
                break;
            case "PAIKECLASSNAME":
                if(value == "") {
                    errorMessage = "上课班级不能为空";
                }else if(value.length>50){
                    errorMessage = "长度最长50个汉字";
                }else{
                    isError = false ;
                }
                break;
  ...
  //添加教室
    $("#roomAdd").find("[class='text']").bind({
        focus:function(){focusItem(this)},
        blur:function(){checkItem(this)}
    });
    $("#roomAdd").submit(checkForm);
