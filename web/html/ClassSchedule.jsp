...
<head>
    ...
    <script type="text/javascript">
        ...
        function ChkValue() {
            ...
            if (chValue) {
                window.location.href = "./paike?action=list&roomVal=" + roomVal + "&id=0";
            }
        }
    </script>
</head>
...
<div id="main" class="wrap">
    ...
    <div class="main">
        <h2>班级课表</h2>
        <div class="manage">
            <td align="center" valign="bottom">班级</td>
            <select id="Sel_ROOM" name="Sel_ROOM" style="height:20px;width:250px">
                ...
                <c:forEach items="${sessionScope.paikeClass}" var="paiKeClass">
                    <option value="${paiKeClass.classId}">${paiKeClass.className}</option>
                </c:forEach>
            </select>

            <input name="btnSearch" type="button" value="检索" class="button" onclick="ChkValue()">
            <c:if test="${not empty sessionScope.paiKeClassList}">
                <table class="list">
                    <tr>
                        ...
                        <tr>
                            ...
                            <c:forEach var="riqi" begin="1" end="7">
                                <td valign="top" width="13%">
                                    <c:forEach items="${sessionScope.paiKeClassList}" var="roomSchedule">
                                        <c:if test="${roomSchedule.paikeJieshu-1 == i && roomSchedule.paikeWeekDay == riqi}">
                                            <a href="${pageContext.request.contextPath}/paike?action=getListById&listId=${roomSchedule.paikeId}">${roomSchedule.paikeCourseName}</a>
